...
const CustomerFieldsPage = () => {
  const theme = useTheme();
  const { showSuccess, showError, showConfirm } = useNotification();
  const { 
    data: customerFields, 
    loading: customerFieldsLoading, 
    error: customerFieldsError, 
    subscribeToData: subscribeToCustomerFields, 
    addDocument: addCustomerField, 
    updateDocument: updateCustomerField, 
    deleteDocument: deleteCustomerField,
  ...
  }, [subscribeToCustomerFields]);

  // Handle errors
  useEffect(() => {
    if (customerFieldsError) {
      showError(`Error loading customer fields: ${customerFieldsError}`);
    }
  }, [customerFieldsError, showError]);
  ...
  const handleToggleStatus = async (field) => {
    try {
      ...
      showSuccess(`Customer field ${newStatus ? 'activated' : 'deactivated'} successfully`);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingField) {
        ...
        await updateCustomerField(editingField.id, fieldData);
        showSuccess('Customer field updated successfully');
      } else {
        // Add new field
        await addCustomerField(fieldData);
        showSuccess('Customer field added successfully');
      }
      ...
      showError('Error saving customer field');
    }
  };
  ...
  const handleDelete = async (fieldId) => {
    showConfirm({
      ...
      message: 'Are you sure you want to delete this customer field? This action cannot be undone.',
      confirmText: 'Delete',
      cancelText: 'Cancel',
      severity: 'error',
      onConfirm: async () => {
        try {
          await deleteCustomerField(fieldId);
          showSuccess('Customer field deleted successfully');
        } catch (error) {
          showError('Error deleting customer field');
        }
      },
    });
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
          ...
          <Card sx={{ borderRadius: 3, boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
            <CardContent>
              ...
              <Typography variant="body2" color="text.secondary">
                Custom fields for individual customers
              </Typography>
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <Card sx={{ borderRadius: 3, boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
            <CardContent>
              ...
              <Typography variant="body2" color="text.secondary">
                Custom fields for corporate customers
              </Typography>
            </CardContent>
          </Card>
        </Grid>
      ...
      {/* Add/Edit Customer Field Dialog */}
      <Dialog 
        open={openDialog} 
      ...
      >
        <DialogTitle sx={{ pb: 1 }}>
          <Typography variant="h6" sx={{ fontWeight: 600 }}>
            {editingField ? 'Edit Customer Field' : 'Add New Customer Field'}
          </Typography>
        </DialogTitle>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                ...
                <TextField
                  ...
                  margin="normal"
                  required
                  placeholder="e.g., Company Name, Vehicle Model"
                  sx={{ mb: 2 }}
                />
        ...
        <DialogActions sx={{ p: 3, pt: 1 }}>
          ...
          >
            {editingField ? 'Update' : 'Add'} Customer Field
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
